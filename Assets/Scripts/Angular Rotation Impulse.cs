...
public class AngularRotationImpulse : ProjectileMotion
{
    ...
    void Update()
    {
        // set inMotion boolean to whatever state it is in the projectileMotion script
        ...
        if (applyRotation == true && inMotion == true)
        {
            ApplyRotationMatrix();
        }
    ...
    {
        Rigidbody projectileRb = proMotion.GetRigidBody();
        ...
        Matrix4x4 currentRotationMatrix = Matrix4x4.TRS(Vector3.zero, projectileRb.rotation, Vector3.one);
        ...
        projectileRb.MoveRotation(newRotation);
    }

    private Quaternion ConvertMatrixToQuaternion(Matrix4x4 matrixToConvert)
    {
        // wasn't usre on how to convert the matrix to quaternion so i looked online and found a unity forum talking about it and used the code found there: https://forum.unity.com/threads/is-it-possible-to-get-a-quaternion-from-a-matrix4x4.142325/
